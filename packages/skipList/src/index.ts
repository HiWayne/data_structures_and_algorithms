...
class SkipList {
  // 跳表的头尾节点,入口
  private head: SkipListNode<null>;
  ...
  public insert(key: number, value: any): void {
    ...
    if (node.key === key) {
      ...
      // 在最底层插入新节点
      let targetNode = new SkipListNode(key, value);
      ...
      while (
        ...
        const targetUpNode = new SkipListNode(key, null);
  ...
  private findNode(key: number): SkipListNode<any> {
    let node = this.head;
    while (true) {
...
class SkipListNode<T> {
  ...
  value: T;
  up?: SkipListNode<any>;
  left?: SkipListNode<any>;
  right?: SkipListNode<any>;
  down?: SkipListNode<any>;
